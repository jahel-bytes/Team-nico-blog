...
import api from "../api";
import { Post, AccessLevel } from "../types";
...
export default function PostEdit() {
  ...
  useEffect(() => {
    api.get(`/posts/${id}/`).then((r) => {
      ...
      setPermissions({
        ...
        authenticated_access: p.authenticated_access,
        team_access: p.team_access,
        owner_access: p.owner_access,
      });
    }).catch(() => setError("Post not found or no edit access."));
  }, [id]);
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/posts/${id}/`, { title, content, ...permissions });
      navigate(`/posts/${id}`);
    } catch {
  ...
  if (error && !post) return <div style={{ color: "#d32f2f" }}>{error}</div>;
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 8, padding: 32, boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
      ...
      <h2 style={{ marginBottom: 24, fontSize: 22 }}>Edit Post</h2>
